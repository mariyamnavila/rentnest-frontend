...
import { Search, ChevronLeft, ChevronRight, DollarSign, ArrowUpDown, CreditCard, X } from 'lucide-react';
import { Input } from '@/components/ui/input';
import { Button } from '@/components/ui/button';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { StatusBadge } from '../shared/StatusBadge';
import type { AdminPayment, AdminUserMeta } from '../../_actions/admin/adminActions';
...
export function AdminPaymentsTable({ payments, meta }: AdminPaymentsTableProps) {
    ...
    <div className="space-y-4">
      ...
      {payments.length > 0 ? (
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-[#e4e4e4] dark:divide-[#2e3440]">
                {payments.map((payment) => (
                  <tr key={payment.id} className="hover:bg-[#fff5f5]/50 dark:hover:bg-[#232733]/50 transition-colors">
                    <td className="py-3.5 px-5">
                      <div className="min-w-0">
                        <p className="text-xs font-bold text-[#222222] dark:text-white truncate">{payment.tenant?.name || 'Unknown'}</p>
                        <p className="text-[10px] text-gray-400 truncate">{payment.tenant?.email}</p>
                      </div>
                    </td>
                    <td className="py-3.5 px-5">
                      <div className="min-w-0">
                        <p className="text-sm font-bold text-[#222222] dark:text-white truncate max-w-50">{payment.rentalRequest?.property?.title || 'Unknown Property'}</p>
                        <p className="text-[10px] text-gray-400 mt-0.5">{payment.rentalRequest?.property?.location}</p>
                      </div>
                    </td>
                    <td className="py-3.5 px-5">
                    ...
                    <td className="py-3.5 px-5">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-xs font-medium text-gray-500 dark:text-slate-400">{payment.method}</span>
                      </div>
                    </td>
                    <td className="py-3.5 px-5">
                      <span className="text-[10px] font-mono text-gray-400 truncate block max-w-32">
        ...
        <div className="text-center py-12 border border-dashed border-[#e4e4e4] dark:border-[#2e3440] rounded-2xl">
          <p className="text-sm text-gray-500">No payments matching your criteria found.</p>
        </div>
      )}
    </div>
  );
