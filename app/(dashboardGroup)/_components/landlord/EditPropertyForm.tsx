...
import { updateProperty, UpdatePropertyState } from '../../_actions/landlord/propertyActions';

type UpdatePropertyFormData = z.infer<typeof createPropertySchema>;
...
export function EditPropertyForm({ propertyId, property, categories }: EditPropertyFormProps) {
  const router = useRouter();
  const [state, action, pending] = useActionState(updateProperty.bind(null, propertyId), initialState);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 sm:space-y-8 font-sans px-2 sm:px-0">
      ...
      {/* Header Banner */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-5 sm:p-8 rounded-3xl bg-linear-to-r from-white via-white to-[#fff5f5] dark:from-[#1a1d24] dark:via-[#1a1d24] dark:to-[#232733] border border-[#e4e4e4] dark:border-[#2e3440] shadow-xs">
        <div className="space-y-1">
          ...
          <h1 className="text-2xl sm:text-3xl font-black text-[#222222] dark:text-white uppercase tracking-tight">
            EDIT <span className="text-[#CFA190]">PROPERTY</span>
          </h1>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <div className="space-y-1.5">
                <label className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-[#222222] dark:text-slate-200">
                  ...
                  Monthly Rate (BDT) <span className="text-rose-500">*</span>
                </label>
                <Input
          ...
          <CardContent className="p-6 py-1 space-y-6">
                ...
                render={({ field }) => (
                  <Select value={field.value} onValueChange={field.onChange} disabled={pending}>
                    ...
                    <SelectContent className="w-(--radix-select-trigger-width) rounded-2xl">
                      {categories.map((cat) => (
                        <SelectItem key={cat.id} value={cat.id} className="w-full rounded-xl cursor-pointer">
                          {cat.name}
                        </SelectItem>
                      ))}
                    </SelectContent>
            ...
            <div className="space-y-2">
              <label className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-[#222222] dark:text-slate-200">
                ...
                Select Amenities <span className="text-rose-500">*</span>
              </label>
              ...
              {amenities.length > 0 && (
                <div className="flex flex-wrap gap-1.5 pt-2">
                  {amenities.map((amenity) => (
                    <span key={amenity} className="inline-flex items-center gap-1 px-3 py-1 rounded-xl bg-[#fff5f5] dark:bg-[#232733] border border-[#CFA190]/30 text-xs font-extrabold text-[#CFA190]">
                      {amenity}
              ...
              {imageUrls.length > 0 ? (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-3">
                  {imageUrls.map((url, idx) => (
                    <div key={idx} className="relative h-24 rounded-2xl overflow-hidden border border-gray-200 dark:border-slate-700 group shadow-xs">
